...
public class MinItemsValidator extends BaseJsonValidator implements JsonValidator {
    ...
    public MinItemsValidator(JsonNodePath schemaLocation, JsonNodePath evaluationPath, JsonNode schemaNode, JsonSchema parentSchema, ValidationContext validationContext) {
        ...
        if (node.isArray()) {
            if (node.size() < min) {
                return Collections.singleton(
                        buildValidationMessage(null, instanceLocation, executionContext.getExecutionConfig().getLocale(), min, node.size()));
            }
        } else if (this.validationContext.getConfig().isTypeLoose()) {
            if (1 < min) {
                return Collections.singleton(
                        buildValidationMessage(null, instanceLocation, executionContext.getExecutionConfig().getLocale(), min, 1));
            }
        }
