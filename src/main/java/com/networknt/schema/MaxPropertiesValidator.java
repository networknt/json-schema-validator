...
public class MaxPropertiesValidator extends BaseJsonValidator implements JsonValidator {
    ...
    public MaxPropertiesValidator(JsonNodePath schemaLocation, JsonNodePath evaluationPath, JsonNode schemaNode, JsonSchema parentSchema,
        ...
        if (node.isObject()) {
            if (node.size() > max) {
                return Collections.singleton(
                        buildValidationMessage(null, instanceLocation, executionContext.getExecutionConfig().getLocale(), "" + max));
            }
        }
